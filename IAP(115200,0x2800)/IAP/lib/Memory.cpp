...
Memory::Memory(uint32_t startAddress,bool useHalfWord)
{
...
///@retval -1 : 读取成功 -0：读取失败
///////////////////////
bool Memory::Read(uint16_t pageNumber, uint32_t* data,u16 length)
{
	u16 dataLength=length;
	if(mUseHalfWord)
	{
...
///////////////////////
bool Memory::Write(uint16_t pageNumber, uint8_t* data,u16 length)
{
	u16 dataLength=length;
	FLASH_Unlock();
...
///@retval -1 : 写入成功 -0：写入失败
///////////////////////
bool Memory::Write(uint16_t pageNumber, uint32_t* data,u16 length)
{
	u16 dataLength=length;
